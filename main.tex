...
\paragraph{A pragmatic path forward.} How to migrate an entire ecosystem? A
viable migration path should abide by four tenets: ({\bf T1}) minimal legacy
code changes, ({\bf T2}) semi-automated migration with limited developer
involvement, ({\bf T3}) testable outcomes, and ({\bf T4}) measurable benefits.\\
We propose a migration path towards a mostly strict language with the following
characteristics. \vspace{1mm}
\begin{compactitem}[---]
...
\item {\bf T3:} The robustness of the inferred signatures under different
...
\item {\bf T4:} One incentive for library developers to adopt strictness
...
\item We evaluate the robustness of the inferred signatures by testing them
  against \todo{XXXX} clients of the packages for which signatures were
  inferred. We report \robustnesResult error rate, \ie signatures that cause
  executions to diverge from the reference semantics.
...
were missing. This excludes the \c{...} parameters which could also be missing,
but are separately discussed above. These \MissingTotalCountArguments arguments
correspond to \MissingTotalCountParameters parameters. Arguments to
\MissingAlwaysCountParameters of these parameters are always missing. These
parameters are classified as \always, in contrast to the
...
\begin{wraptable}{r}{6cm}
  ...
  \begin{tabular}{lrrr}
    ...
    \midrule
    {Parameters}&\SometimesCountParameters&\NeverCountParameters&\UnevaluatedCountParameters\\
    {Functions}&\SometimesCountFunctions&\NeverCountFunctions&\UnevaluatedCountFunctions\\
    {Packages}&\SometimesCountPackages&\NeverCountPackages&\UnevaluatedCountPackages\\
    \bottomrule
  \end{tabular}
\end{wraptable}

Of the 294M total arguments in our corpus, \UnevaluatedCountArguments of
promises were not evaluated. They correspond to \UnevaluatedCountParameters
parameters from \UnevaluatedCountFunctions functions of
\UnevaluatedCountPackages packages. We can classify these parameters into two
...
\SometimesCountParameters \sometimes parameters and \NeverCountParameters \never
parameters. These may correspond to code paths not exercised or to dummy
parameters.
%
...
Two parameters from two functions, \code{R.oo::.getFunctionByName}, and
