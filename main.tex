...
\vspace{1mm}
\begin{compactitem}[---]
\item {\bf T1:} Our extensions to the R syntax and semantics are non-invasive.
  The behavior expected by most users closely aligns with strict semantics. Our
...
side-effects and reflective operations, adding extra complexity to our
inference algorithm.
...
file is run a separate script. Examples and vignettes are snippets of code
...
The package in the corpus have 50,435 top-level functions We ignore anonymous
...
signals (M, U, S, R) for each individual parameter. There were 9,100 parameters
that were marked M for meta-programmed. Table~\ref{table:strictdist} summarizes
the distribution of combination of the other signals; note that functions and
...
instance, shows that 128K parameters coming from 44K functions and 489 package
were not marked in any way. The second row shows that only 134 for parameters
...
are always evaluated and the corresponding arguments do not perform side-effects
or reflective operations. They can become strict. Of the remaining, there are
13\% unevaluated parameters, the largest potential source of laziness. This is
followed by 4\% of parameters whose arguments are marked S only. Everything else
is quite small. Another observation, is that rows marked R come from very few
...
it was not addressed there. The macro is used by the bultin functions of the GNU
R interpreter, a canonical example is the \code{missing} function to check if an
argument was provided. Many packages also use \code{PREXPR} to extract a
...
\noindent
Another pattern is to delay evaluation of a parameter.
...
Of all the promised observed by \lazr, only 0.5\% perform side-effects. Many
...
arguments in our corpus. Majority of arguments, 99.5\% to be precise, do not
...
side-effect using \code{<<-}, this is a common way to update global state.
%
\begin{lstlisting}
...
to a variable \code{.N} in the caller's environment. Eventually, it removes the
variable.
%
\begin{lstlisting}
...
by \lazr. This done by running scripts extracted from 2000 client packages of
our corpus. These packages have 4.5M lines of R code and 4.7M lines of native
code. Table ~\ref{table:clientcorpus} gives numbers scripts and lines of code
exercised.
...
Comparing output of scripts is standard practice in the R community for
detecting regressions. There may be difference in execution that are not
...
parameter marked U or S must be lazy, and configure $-U+S+R$ means that
parameters marked S and R must be lazy.

\cconfig 0 is the most lazy of the configurations and only 239 scripts out of
42K have erroneous outputs. The reason for these errors comes from some of the
...
hides 5 errors. This is an outlier that should be elucidated.
...
One takeway from this evaluation is that unevaluated parameters (U) have only a
