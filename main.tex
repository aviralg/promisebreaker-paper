...
\begin{abstract}
  ...
  infers strictness signatures for functions to capture both intentional and
  accidental laziness.
...
%%  arguments that have been annotated as strict.
  ...
  packages by eagerly evaluating the indicated arguments and found only
  \robustnesResult of the test to fail.  Finally, we report on
  performance benefits of eager evaluation on a just-in-time compiler for R,
  where we achieved an average speedup of \speedupRshStrict.
\end{abstract}
...
\item We introduce \strictr, a non-invasive extension of the R language, where
  ...
  compatible with the package loading model. In the reference implementation GNU R
  the proposed semantic is implemented using instrumentation.
\item We implement \lazr, an infrastructure for large-scale inference of
  ...
  for the api. The process is shown to scale up to the 500
...
\item We evaluate the robustness of the inferred signatures by testing them against
  ...
  signatures were inferred. We report \robustnesResult error rate, \ie signatures
...
\noindent Our conclusions are that a semi-automated migration is viable; for community
...
\begin{itemize}
\item[] {\hskip -1em \bf\small\code{delayedAssign(x,exp,eenv,aenv)}}: creates a promise
...
\item[] {\hskip -1em \bf\small\code{substitute(e,env)}}: substitutes variables in \code e
  with their values found in environment \code{env}, returns an
  \code{expression} (a parse tree).
\item[] {\hskip -1em \bf\small\code{force(x)}}: forces the promise \code x. This replaces a
  common programming idiom, \code{x<-x}, which forces \code x by assigning it
  to itself.
\item[] {\hskip -1em \bf\small\code{forceAndCall(n,f,...)}}: call \code{f} with the
...
%% comment (o): here I don't mention ... on purpose, because I think it is just
%% a limitation of the approach and not a laziness signal...
...
address.
%% Cache are updated on allocation and deallocation events. This prevents
...
execution traces obtained from their examples, tests, and vignettes. For
validation, we selected 2000 dependent packages of the synthesis corpus. They
have 4.5M lines of R code and 4.7M lines of native (C, C++, and Fortran) code.
We run their examples, tests, and vignettes to exercise the functions with
signatures. This allows us to asses the amount of breakage introduced by \lazr
strictness signatures in client code. Table ~\ref{table:corpus} gives the number
of scripts of each kind that were run and lines of code exercised.
...
Our analysis pipeline starts with setting up a
container image that includes all the dependencies for installing analysis code and
R packages. This
...
followed by generation of execution traces. These traces are analyzed to generate
...
For validation, we selected 2000 dependent packages of the synthesis corpus.
They have 4.5M lines of R code and 4.7M lines of native (C, C++, and Fortran)
code. We run their examples, tests, and vignettes twice without any signatures
to filter programs which finish successfully with the same output in both runs.
This provides us with a baseline set of deterministic programs.
...
percentage of failing tests, vignettes, examples, and tests.
