...
Here, \c{expr} may be delayed until \c{pkg} is loaded. Thus, \c{expr}
...
In the following example, the first argument is always evaluated, but \c{n}
and \c{m} are only evaluated sometimes and \c{r} never.
%
\begin{lstlisting}
...
interface, but not all functions need all parameters. The \c{proxy} package
defines over a dozen methods with the interface \c{function(a,b,c,d,n)} that
compute different proximity metrics using a subset of the arguments.

Arguments can be \never by design; \c{tail} is not defined for
\c{tbl_lazy} objects and throws an error when called.
\begin{lstlisting}
...
not have the correct type. In all observed calls, \c{signal}
went  unused because \c{e} had the right type.
\begin{lstlisting}
