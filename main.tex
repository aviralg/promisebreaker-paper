...
by \lazr. This is done by running scripts extracted from client packages of our
corpus. Our corpus of 500 packages has \CorpusDependencyCount client packages in
CRAN. From these, we select the top 2000 client packages for this experiment.
These client packages have \ClientRCode lines of R code and \ClientNativeCode
lines of native code. Table ~\ref{table:clientcorpus} gives the number of
scripts and lines of code exercised. Using \lazr, we extracted \todo{Add script
  count} scripts from the clients. From these, we filtered scripts whose output
was non-deterministic, leaving us with \todo{non-deterministic script counts}
scripts. The goal of the next step is to validate if, after applying strictness
signatures, they produce identical outputs. Comparing the output of scripts is
standard practice in the R community for detecting regressions. There may be a
difference in execution that does not manifest in the output; this may hide some
errors, so the results reported here should be considered a lower bound.
...
With all the signatures applied, we observed \todo{XX} scripts produce
erroneous output. We narrowed down the cause of these errors to a handful of
packages: \code{R.oo}, \code{R.utils}, \code{vctrs}, \code{ggplot2},
\code{Matrix}, and \code{spam}. Removing the signatures of these packages
resulted in only \todo{XX} failing scripts. We discuss below the cause
of failures from these packages.

Loading package \code{R.oo}, and \code{R.utils} with the signatures terminates
the programs with the error shown in Figure~\ref{fig:roo}. Both of these
packages have the same author. The \code{R.oo} package implements an
object-oriented system. The strict semantics interfere with the initialization
...
\begin{figure}[H]
  ...
  \caption{Error from \code{R.oo}}\label{fig:roo}
\end{figure}

Loading package \code{vctrs} with the signatures terminates the programs with
non-deterministic error shown in Figure~\ref{fig:vctrs} originating from native
code. We believe that the strict semantics expose a memory bug in this package.
Running the program a couple of times gets rid of the error.
...
\begin{figure}[H]
  ...
  \caption{Error from \code{vctrs}}\label{fig:vctrs}
\end{figure}

Loading package \code{Matrix} changes the printout message on package loading.
The message is shown in Figure~\ref{fig:matrix} with the difference colored in
red. We studied the output of a few clients of this package but could not find
any other discernible difference.
...
\begin{figure}[H]
  ...
  \texttt{
    ...
    \textcolor{red}{Attaching package: ‘Matrix’\\
    The following object is masked from ‘package:base’:\\
    det}
  }
  \caption{Different output from \code{Matrix}}\label{fig:matrix}
\end{figure}

The \code{spam} package similarly provides a \code{det} function and prints a
different startup message as shown in Figure~\ref{fig:spam} with the signatures
applied.
...
\begin{figure}[H]
  ...
  \texttt{
    ...
    backsolve, \textcolor{red}{det,} forwardsolve
  }
  \caption{Different output from \code{spam}}\label{fig:spam}
\end{figure}

The results suggest that the semi-automated inference algorithm of \lazr works
well for the most part. Only six packages are responsible for most of the
errors. Fixing these errors requires inspection of the signatures by the
