...
\code{3*x}, and which are only evaluated when needed. The function returns a
closure.
...
\noindent
This function can be called with a single argument matching \code x, as in
...
parameter \code{x} was provided at the call site or not, even if it was later
substituted by a default value. A vararg parameter, written \code{...}, accepts
an arbitrary number of arguments, including missing arguments. A vararg can be
materialized into a list with \code{list(...)}. Most frequently varargs are
...
R supports meta-programming. The \code{substitute(exp,env)} function yields the
parse tree of the expression \code{exp} after performing substitutions defined
by the bindings in \code{env}. Consider the following call that substitutes 1
...
first-class objects. They are evaluated using the \code{eval(exp,env)} function.
...
imperative features such as assignment to local variables \code{<-}, assignment
to variables in an enclosing scope \code{<<-}, and assignment in a
programmatically chosen scope \code{assign()}. R supports non-local returns
either through exceptions or by delayed evaluation of a \code{return} statement.
...
takes an argument \code x and returns \code{x+x}. When called with an argument
...
\item[] {\hskip -1em \bf\small\code{delayedAssign(x,exp,env,aenv)}}: creates a
  promise with body \code{exp} and binds it to variable \code x (here \code x is
...
\item[] {\hskip -1em \bf\small\code{substitute(e,env)}}: substitutes variables
  ...
  \c{expression}.
\item[] {\hskip -1em \bf\small\code{force(x)}}: forces evaluation of its
  argument. This replaces a common programming idiom, \code{x<-x}, which forces
  \code x by assigning it to itself.
...
For details about \code{substitute}, we refer the reader to \cite{oopsla19b}.
We elaborate on \code{PREXPR} since it was not addressed there. This macro is
used by many packages to extract a promise's expression for ad-hoc evaluation
strategies. We found it in packages like \code{lazyeval} and \code{rlang}. The
latter stands out because its own API for meta-programming. The \code{PREXPR}
macro is also used by the builtins of the GNU R interpreter, a canonical example
is the \code{missing} function which checks if an argument was provided. Unlike
user packages, these uses of \code{PREXPR} don't require the corresponding
arguments to be lazily evaluated. Hence, uses of \code{PREXPR} from GNU R
...
\code{y} a \sometimes parameter. Another pattern is to delay the evaluation of a
parameter.
