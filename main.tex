...
\section{StrictR for an R without Lazy}
...
\section{Identify the Lazy}

This section presents our dynamic analysis \lazr. We use it to infer arguments
that need to be lazy from a corpus of R packages.
\lazr is an analysis pipeline for inferring strictness annotations in legacy R
libraries at scale. It builds on the \rdyn package which extends the GNU R
virtual machine version 4.0.2 with hooks for registering callbacks~\cite{oopsla19b}.
%%
%% \subsection{Infrastructure}
%% 
%% The analysis pipeline starts with setting up a
...
%%
The low-level \rdyn provides hooks into the R interpreter to observe various
events.
%% allows registering callbacks with raw R objects at various
...
%% 
\lazr provides a layer of abstraction on top, dealing with issues such as object
identity, and references.
%% 
%% of the event framework. It allows
...
%% 
\lazr also keeps track of logical time, which is incremented on every event.
This gives us a total order of all events and we can for instance place effects
relative to the birth and death of promise objects. The abstractions include
access and modification times of environments, as well as start and end of
promise evaluation.
...
promise. Furthermore, \lazr ensures the correct order of events in the presence
of non-local returns, and similar exceptional controlflow, that is implemented
using \code{longjmp} in GNU R. Finally, it provides unique names for
environments and functions.
%% function names are also modeled. Getting
%%fully qualified function names dynamically is challenging in R
These need to be managed, because package namespaces are first class
environments that are constructed piecemeal and can still be mutated.
\lazr assigns names to environments on package load
events and records the associated names when functions are added.
It also records the nesting of inner functions to arbitrary depths.
During execution, \lazr collects information about function calls, arguments,
side-effects, and reflective environment access. In particular it records the
following events: function entry and exit, start and end of promise evaluation,
reads and writes to variables, mutation of promises, and lookups of functions.
This information is stored in compressed tabular format using R's \c{fst} library.

\paragraph{Algorithm}

The execution traces gathered by \lazr tell us if an argument
...
execution traces to get a summary for each function parameter
position. It uses this summary to decide if an argument can be evaluated eagerly
and generates strictness signatures. The signatures also dictate the order of
evaluation of arguments. The potential sources of lazyness identified by \lazr
are:

\begin{description}
  \item[Ellipsis]  TODO
  \item[Metaprogramming] TODO
  \item[Unevaluated arguments] TODO
  \item[Are these all] ??? \OF{these should mirror the ones you discuss in the
    next subsection}
\end{description}
...
meta-programming relies on unevaluated argument text provided by a promise.
For an in-depth discussion on the patterns of use of \code{substitute}, we refer
the reader to \citet{oopsla19b}. We additionally investigate the use of
\code{PREXPR} for metaprogramming, since that is not addressed in
\cite{oopsla19b}.
The remaining operations are sources of \emph{accidental laziness}.
\OF{This cries for a table of the different configurations possible} We could decide
...
operation in the programs it has run. \OF{I guess that's only the case for this particular
configuraion} Including more programs will make more
arguments lazy.
...
%% generates signatures for different strictness configurations.
We output one file
...
pipeline.
%% Each analysis is split in 4 phases. The first phase is the
...
\subsection{Inferring Lazyness with \lazr} \label{subsection:lazr}

We used \lazr to infer strictness signature for a large corpus of R packages.
In the reminder of the section we present our findings and discuss the patterns we identified
in the data.
...
We selected our corpus from packages and client programs from CRAN and Bioconductor, two
...
We analyzed functions from 500 packages. The criterion was to
select packages which had the most clients,\ie packages which are the
most widely used in the ecosystem. This requirement will prove handy for
evaluation \OF{but it also diminishes your claim that this will work for the
ecosystem at large, since typically packages will have fewer clients...}.
The chosen packages have a total of 15,362 dependent client
...
the highest number of parameters. \OF{put these in a table}
...
\paragraph{Orders of Evaluation}
...
\subsection{Origins of Lazy Arguments}
...
The majority of parameters are \always parameters, suggesting that arguments passed
...
\OF{maybe this should be discussion?}
\OF{should this section move to robustness? in a sense it is estimating how
plausible the signatures are...}
...
\OF{Don't end paragraph in listings}

\paragraph{Reflection}

As already mentioned, a common source of parameters which need to be lazy is the
use of reflection. A particulary interesting example is presented by
...
\OF{We need some sort of a conclusion here....}

\section{Robustness}

To validate the signatures described in the previous section,
we selected 2000 dependent packages of the synthesis
...
the reduced allocation rate. We measured the difference in garbage collection
time and to our surprise found that it is miniscule, ranging from
\speedupGCRshStrictMin to \speedupGCRshStrictMax.
...
use at all. In this variant benchmarks slowed down by TODO
\slowdownFromPromiseAlloc... \OF{discuss with actual results}.
