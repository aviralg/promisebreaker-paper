...
\section{Compiling the Lazy Away}

In this section we report on our experiments to evaluate the implications of
eagerness on implementations of R. To that end we modified \Rsh, a just-in-time
compiler for the language. \Rsh features a two tier compilation strategy. The
first tier is realized by a naive bytecode interpreter, the second tier
by an optimizing native compiler. This optimizer employs
speculative inlining of R functions and promises. We changed the first-tier
compiler to eagerly evaluate all arguments at all call-sites, except for a
manually curated list of exceptions. Additionally, we changed the optimizing
compiler to inline the expressions of default arguments, which are normally also
wrapped in promises and evaluated by need. In the following we will refer
...
the suite, that preserves the means and outliers, and includes one variant of
each benchmark. As predicted by (H2) only a handful of functions, such as
\lstinline{tryCatch} had to be kept lazy for all the benchmarks to compute
the correct results. Based on the observations so far we
pose the following predictions: eager semantics should improve performance of
most benchmarks; a significant portion of the speedup is due to reduced garbage
collection pressure; and there is an additiona speedup due to improved compiler
optimizations.
...
\begin{figure}
  ...
  \caption{Speedup of \Rshstrict gained by eagerly evaluating arguments}
  \label{fig:speedup}
\end{figure}
...
\paragraph{GC Pressure}

\begin{figure}
  \centering
  \input{graphs/rshStrictPerf.tex}
  \caption{TODO gcpressure}
  \label{fig:gc-pressure}
\end{figure}

Even an optimizing compiler has to allocate many promises for R
code. Thus we expect a large portion of the speedup to actually originate from
the reduced allocation rate. We measure the number of promises allocated in the
benchmark suite
in GNU R, in Ř and in Ř-strict. The results are shown in
\autoref{fig:gc-pressure}. The first step from a naive interpreter to an
...
still relatively high in some cases. As far as we were able to observe, the
...
\begin{figure}
  \centering
  \input{graphs/rshStrictPerf.tex}
  \caption{TODO speedup due to gc}
  \label{fig:gc-pressure2}
\end{figure}


Finally, we measured the time spent in garbage collection in \Rsh and in
\Rshstrict with $t_e = 1, t_i = 15$. The results are show in
\autoref{fig:gc-pressure2}. Overal, we found that about \speedupDueToGCPressure
of the speedup is explained by reduced GC pause times.
...
beyond what can be explained by the reduction in GC pressure. To measure this
effect we disabled the optimizing native compiler in \Rsh, such that all code is
executed in the first-tier bytecode interpreter. Then we repeated the
performance experiment from \autoref{fig:speedup} in that configuration, with $t_e = 1, t_i = 15$.
The results are presented in \autoref{fig:speedup-bc}. We found that the bytecode
interpreter gains a speedup of \speedupBCRshStrict, ranging from
\speedupBCRshStrictMin to \speedupBCRshStrictMax. Compared to the full system,
which sped up by \speedupRshStrict, we see slightly less gains in performance.
We conclude that there is only weak evidence for the optimizing compiler to benefit
more from eager semantics than the interpreter.
