...
Calls to \code{R.oo:::.getFunctionByName} result in transitively making 18
parameters from 15 functions of 3 packages lazy. We manually analyzed the call
sites and found that they are identified lazy because our algorithm is
conservative. When it finds a reflective call from an argument promise, it
taints all the function arguments on the stack. None of these functions pass
their arguments containing calls to \code{as.environment(-1)} or
\code{pos.to.env(-1)} to \code{R.oo:::.getFunctionByName}.
