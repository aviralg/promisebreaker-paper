...
\begin{figure}[h!]
  ...
  \scalebox{0.85}{
    \begin{tikzpicture}
      ...
      \tikzstyle{block}     = [rectangle, minimum width=.08 \textwidth, minimum height=35pt]
      \tikzstyle{connector} = [line width=0.25mm, ->]

      \node [block, fill = maroon]                                (install)   {\nodename{Install\\CRAN}};
      \node [block, right = \nodesep of install,   fill = yellow] (extract)   {\nodename{Extract\\Programs}};
      \node [block, right = \nodesep of extract,   fill = blue]   (trace)     {\nodename{Trace}};
      \node [block, right = \nodesep of trace,     fill = green]  (reduce)    {\nodename{Reduce}};
      \node [block, right = \nodesep of reduce,    fill = orange] (combine)   {\nodename{Combine}};
      \node [block, right = \nodesep of combine,   fill = red]    (summarize) {\nodename{Summarize}};
      \node [block, right = \nodesep of summarize, fill = purple] (report)    {\nodename{Report}};
      ...
      \begin{scope}[on background layer]
        \node[fit=(install) (extract), rectangle, fill=gray, fill opacity=0.8, label=above:\textbf{Corpus}]          (corpus) {};
        \node[fit=(trace),             rectangle, fill=gray, fill opacity=0.8, label=above:\textbf{Tracing}]         (exectrace) {};
        \node[fit=(reduce) (report),   rectangle, fill=gray, fill opacity=0.8, label=above:\textbf{Post-processing}] (postprocessing) {};
      \end{scope}
    \end{tikzpicture}
  }
...
\begin{figure}[th!]
  ...
  \scalebox{0.85}{
    \begin{tikzpicture}
      ...
      \tikzstyle{block}      = [rectangle, minimum width=.1 \textwidth, minimum height=35pt]
      \tikzstyle{dummyblock} = [rectangle, minimum width=.1 \textwidth, minimum height=23pt]
      ...
      \node [block, fill = green]                                                           (corpus)            {\nodename{Corpus}};
      \node [block, right = \nodesep of corpus, fill = gray]                                (dummyrun)          {};
      \node [block, above = \dummynodesep of dummyrun.north, anchor = north, fill = yellow] (runone)            {\nodename{First\\Run}};
      \node [block, below = \dummynodesep of dummyrun.south, anchor = south, fill = yellow] (runtwo)            {\nodename{Second\\Run}};
      \node [block, right = \nodesep of dummyrun, fill = blue]                              (outone)            {\nodename{Compare\\Output}};
      \node [block, right = \nodesep of outone, fill = purple]                              (strictrun)         {\nodename{Run with\\StrictR}};
      \node [dummyblock, above = 0mm of strictrun.north, fill = gray]                       (dummystrictrun)    {};
      \node [dummyblock, below = 0mm of strictrun.south, fill = gray]                       (dummystrictruntwo) {};
      \node [block, right = \nodesep of strictrun, fill = red]                              (outtwo)            {\nodename{Compare\\Output}};
      ...
      \begin{scope}[on background layer]
        \node[fit=(corpus) (runone) (runtwo) (outone),                       rectangle, fill=gray, fill opacity=0.8, label=above:\textbf{Deterministic Corpus}] (detcorpus) {};
        \node[fit=(strictrun) (dummystrictrun) (dummystrictruntwo) (outtwo), rectangle, fill=gray, fill opacity=0.8, label=above:\textbf{Failures}] (failures) {};
      \end{scope}
