...
\section{A Strict-by-default compiler for R}\label{sec:rsh1}
...
run, R implementations should generate native code.

The alternative was to use an experimental native compiler. There are two
available in open source, Oracle's Truffle/R implementation~\cite{} and our own
\Rsh~\cite{}. Both systems have their drawbacks. Truffle/R is not a complete
implementation of the language and it is unfamiliar to the authors. \Rsh is less
mature, in terms of optimizations, but runs all R programs. We convenience, we
picked \Rsh as a starting point.
