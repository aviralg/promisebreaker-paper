...
are inlined in the first-tier compiler of Ř. Overal we observe a mean speedup up
xx\% and no significan regressions.
...
optimizing compiler leads to an xx\% reduction in allocations. The second step
from lazy to eager semantics to an xx\% reduction. Overal, the reduction of
promises is on average xx\%. Surprisingly the number of remaining promises is
still relatively high. \OF{TODO: where do they come from?} Also, overall xx\% of the
