...
libraries at scale. \lazr has several components, there is system for tracing
the execution of R scripts and an infrastructure for extracting executables and
running analyses that scales to thousands of packages.

The goal of the analysis performed by \lazr is not to infer maximally strict
signatures for all library functions, but rather to infer signatures which will
...
variable \c c lazy will return the same result. For \c{r2}, if \c a and \c b are
...
observed. And finally, for \c{r4} it is \c{e} that must be lazy.
...
new code, we expect programmers to specify, in their documentation what needs to
be lazy, and mostly use strict parameters.

\subsection{Tracing the execution of R scripts}

The heart of \lazr is dynamic analysis tool built on the \rdyn package which
...
abstract from the concrete R data structures. The model objects are used to
represent function instances, the hierarchy of environments, frames on the call
stack, amongst other. Model objects are particularly helpful to handle some of
the complexities of R. For instance, model objects have unique identities,
whereas R objects are identified by their memory address, and these can be
reused by the garbage collector. \lazr takes care of memory management for model
objects and provide efficient indexing. Model functions have names that
heuristically reconstructed and keep track of their lexical scoping structure.
Model stack can deal with R's pervasive use of \c{longjump} for non-local
returns.
Lastly, \lazr maintains a notion of logical time which is used to record when
some events of interest happened. For instance, model environments record the
time of last read and last write. Similarly, functions and promises have the
start and end of their evaluation recorded. Logical time is used, for example,
to decide if an event happened while a promise was executing, or the ordering of
events such as reads and writes.
...
parameter must be lazily evaluated. These are the signals that we use:
\begin{itemize}
...
identify laziness required to preserve the result, regardless whether it is
accidental or not.
...
operations that are related to the evaluation of promises and correlate them
...
For any write, \lazr check if their is an active promise on the call-stack, if
not the write is ignored. Then it checks if the write is \emph{visible}, a write
is only visible if the environment being updated pre-dates the promise being
evaluated. Writes that are not visible, are ephemeral and cannot be observed. If
the write is visible, then the active promise is marked as the \emph{direct}
cause of the effect and the event is recorded for later analysis. If there are
multiple promises being evaluated, only the first of those is the direct cause,
others are \emph{transitive}.
...
When an error or exception is encountered, \lazr marks all active promises as
causes.
...
When a promise calls \code{as.environment(v)} or and \code{pos.to.end(v)}, and
...
The result of analyzing a single R script is a table summarizing, for each
function and each argument which signals were observed. The result of analysis
of multiple scripts can be straightforwardly merged as the union of signals for
each parameter.
...
an function on the output of one trace to get a partial summary. The combine
...
size. Finally, in the report phase creates graphs, and tables for inclusion in
the paper.
...
by \lazr. We run client programs to exercise the functions with signatures. The
client programs are the tests, examples, and vignettes from 2000 dependent
packages of the 500 packages from the synthesis corpus. These 2000 packages have
4.5M lines of R code and 4.7M lines of native (C, C++, and Fortran) code. Table
...
effectiveness of our signatures. We extracted 49,484 runnable programs from the
2000 client packages from which 42,637 programs yielded the same result on both
runs. These are the programs for which we present the outcome of applying our
signatures.

%% NOTE: is this useful discussion?
We consider an application of signature successful if it produces the same
output as it produced without applying any signature. We believe that programs
that produce the same output with and without signatures compute the same
result. The rationale or this is that the top-level expressions in R scripts
print their result on the console unless it is stored in a variable or
...
shadowing, warnings, and errors. A simple heuristic to test this claim is to
...
output. In our baseline deterministic programs, 55.8\% had unique outputs.
...
After identifying the baseline programs, we run them eight times, once for each
signature configuration, and count programs that fail or produce a different
output. The result of applying signatures to the deterministic programs are
summarized in Table~\ref{table:strictfail}. Each row represents a signature
configuration. For each configuration we report the number and percentage of
failing programs, further broken down into failures per script type. The letters
\emph{U}, \emph{S}, and \emph{R} represent unevaluated, side-effecting and
reflective arguments. The \emph{+} sign signifies that the corresponding
arguments are treated lazy in the signature configuration and \emph{-} sign
means they are evaluated eagerly. For example, $+U+E+R$ means that arguments
that are unevaluated, side-effecting, and reflective are treated lazy whereas
$-U-E-R$ means that these arguments are evaluated strictly. Each row of this
table directly corresponds to a row of Table~\ref{table:strictdist}. For
example, from row \#1 of both tables, we can say the \config{1} will evaluate
134 arguments strictly over and above the arguments which were already strict in
the minimally lazy configuration.

Now, we take a deeper look at Table~\ref{table:strictfail} for insights.
\cconfig{0} corresponds to the configuration with maximum laziness and it
failure rate is only 0.56\%. We can use this as a baseline to study the impact
of making various arguments categories strict.
...
From the results, we conclude that in the minimally strict configuration, we
have least number of failures. A first attempt at removing laziness at scale can
rely on this signature configuration. Next, reflective and unevaluated arguments
can be made strict without a significant increase in failure rate.
Side-effecting arguments result in many failures. Evaluating arguments which are
both side-effecting and reflective results in maximum failures . However,
Table~\ref{strictdist} tells that these arguments are very few in number. These
arguments can perhaps be left lazy.
...
evaluated. They correspond to 28.3K parameters of 12.1K functions.
%
Based on argument evaluation, we can classify these parameter positions into two
categories: \sometimes parameters whose arguments are evaluated at least once
but not always, and \never parameters whose arguments are never evaluated. There
are 16.4K \sometimes parameters and 11.9K \never parameters. Their presence
indicates paths not exercised by inputs or that they are dummy parameters.
...
Our algorithm identifies 2.7K parameters as transitively lazy. As discussed
before, \lazr traces off precision for scalability. However, impact of this
imprecision is limited.
...
%
\begin{lstlisting}
backports::get0 <- function (x, envir = pos.to.env(-1L), ...)  {
    ...
    mget(x[1L], envir = envir, ...)[[1L]]
    ...
}
\end{lstlisting}
%
