...
arguments are always lazy. Metaprogrammed arguments are also lazy as
...
Table~\ref{table:strictdist} presents a summary of strictness; the number of
strict parameters, functions, and the packages in which these functions are
defined; for all combinations of the sources. If laziness from a source is taken
into account, it is marked with a \xmark{}, otherwise, it is marked with a
\cmark{}. We observe that row \#0 corresponds to the configuration with maximum
laziness, all three sources are taken into account for synthesizing signatures.
Row \#7 corresponds to maximum strictness, all three sources are discarded and
only metaprogrammed and $...$ arguments are treated lazy.
