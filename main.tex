...
callee's parameters and their default values. Assigning a single strictness
annotation to $...$ is tricky because a function can have different strictness
behavior for the individual arguments. One solution is to treat the function
strict in $...$, if it is strict in all of its constituents. However, assigning
a unique strictness signature to $...$ is difficult because a function may use
them in a strict setting but the callees may metaprogram on the promises
packaged inside $...$. This is possible because unlike normal argument promises,
promises inside $...$ are forwarded as-is to callees. Normal promise arguments
are packaged in a new promise before being passed to a callee. In general, a
combination of $...$ and other arguments passed by a caller can match against a
callee's $...$ and the callee could pass this further resulting in a long chain
of functions having access to the same argument promise. Since the callees can
directly affect these arguments, their strictness signature assignment becomes
quite brittle. The presence of missing arguments inside $...$ introduces another
quirk. In light of all these oddities, we treat $...$ as lazy.
