...
We observe XXX arguments created in our execution traces. Of those XXX are $...$
arguments, XXX are missing, XXX are promises, and XXX are R values. R values are
seen rarely, they are an artifact of R's internal implementation. These
arguments correspond to XXX parameter positions fo the XXX functions for which
we generate signatures. Figure ~\ref{fig:paramDist} shows the distribution of
these parameters per function. 7.3\% functions have 0 parameters, 19.6\% have 1,
and 5.0\% have over 10. There are 15 functions with over 50 parameters that come
...
\emph{rpart.plot::check.if.dot.arg.supported.by.rpart.rules} (122).

Function \emph{rpart.plot::check.if.dot.arg.supported.by.rpart.rules} has the
the highest, 122 parameters.
