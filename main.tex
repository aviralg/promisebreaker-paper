...
default values.
%%% \SK{shouldn't we sayz something about z here?}
%%% TODO -- I am not sure what (Says Jan)
...
varargs and 271M are promises. These arguments correspond to 186K parameter
...
their distribution. One functions can be counted in multiple rows as its
...
\c{lazyeval} and \c{rlang} uses it. \c{PREXPR} is also used by the builtins of
...
\begin{wraptable}{r}{6cm}\small\centering
  ...
  \begin{tabular}{lrrr} \toprule
    ...
    {Packages}&\MissingSometimesCountPackages&\MissingAlwaysCountPackages&\MissingTotalCountPackages\\   \bottomrule
  \end{tabular}
\end{wraptable}

\paragraph{Missing Arguments}
Some \MissingAlwaysCountParameters parameters are always missing; they are
classified as \always. In contrast to the \MissingSometimesCountParameters
\sometimes parameters which are sometimes passed an argument. Numbers for these
two categories are in Table~\ref{table:missing}. The \always parameters are a
strong signal, whereas the \sometimes parameters are only lazy if they are also
marked U. We show an \always parameter next; the \c{names} argument was missing
in all the calls to this function defined in the \c{rlang} package. This is no
surprise as the function does not use this argument, it is presumably there for
...
\paragraph{Unevaluated Arguments}
...
is to one argument only if another has a given value.
...
S3 generic methods are functions dynamically dispatch: here, \c x is always
...
Some functions implements a common interface, the \c{proxy} package defines over
...
they happen to local variables of the promise. Table~\ref{table:effects} counts
lookups (L), definitions (D), assignments (A), and removals (R), and the
arguments, parameters and functions responsible for those effects. Lookups are
the most common cause for making arguments lazy. This is followed by definitions
and assignments. Removing bindings from environments is not a common operation.
...
A common source of observable writes is the \c{shiny} package used for building
interactive web applications: \c{bindEvent} attaches an observer expression to
an event. The observer performs a non-local side-effect using \c{<<-} to update
global state.
...
Many side-effects occur in test cases written with the \c{testthat} library. The
global state is modified temporarily to test its effect on the function under
consideration. Another common pattern is when the result of an intermediate
computation is assigned to a variable for subsequent use. In the call below, the
intermediate result is assigned to \c{x} before being passed on to the
...
of our corpus. These packages have 4.5M lines of R code and 4.7M lines of native
code. Table ~\ref{table:clientcorpus} gives the number of scripts and lines of
code exercised. Using \lazr, we extracted 52K scripts from the clients. From
these, we filtered scripts whose output was non-deterministic, leaving us with
42K scripts. The goal of the next step is to validate if, after applying
strictness signatures, they produce identical outputs. Comparing the output of
scripts is standard practice in the R community for detecting regressions. There
may be a difference in execution that does not manifest in the output; this may
hide some errors, so the results reported here should be considered a lower
bound. Fig.~\ref{pipe} shows the full pipeline, it starts with extracting
...
signal to treat as requiring lazy evaluation. Table~\ref{table:strictfail}
