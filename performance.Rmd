...
H3.2a.norm <- function(prom) {
  ...
  rects <- data.frame(xstart = seq(0.5,13.5,1), xend = seq(1.5,14.5,1), 
                      benchmark = selection)
  prom_plot <-
    ...
    scale_fill_grey(start = 0.2, end = 0.6) +
    coord_flip() +
    geom_text(aes(label = ifelse(is.na(gnur), "", format(gnur, big.mark = ","))),
              y = 1.0, hjust = 0, vjust = 0, size = 1.6, angle = 15) +
...
ggplot(mtcars, aes(x = wt, y = mpg)) + 
  geom_point() +
  geom_ribbon(aes(xmin=0, xmax=1), alpha=0.2) +
  theme_minimal()
```
