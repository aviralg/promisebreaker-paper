...
optimizerExperiment <- list(
  `Ř`               = performanceExperiment[[1]],
  `Ř no-opt`        = NA,                          # rir ???
  `Ř strict`        = performanceExperiment[[2]],
  `Ř strict no-opt` = NA                           # rir ???
)
```
...
fetch.gitlab.gc <- function(job) {
  ...
  url.gitlab.suffix <- "/artifacts/file/dataGC.csv"
  dir.create("data", showWarnings=F)
  setwd("data")
  file <- paste0(job, ".csv")
  if (!file.exists(file))
    download.file(paste0(url.gitlab.start, job, url.bmdash.suffix), file)
  res <- read.csv(file, header=TRUE, strip.white=TRUE)
  res$row <- 1:nrow(res)
  setwd("..")
  res
...
# opti <- fetch_jobs(optimizerExperiment, fetch.bmdash)
```
...
H3.1 <- function(perf, kind) {
  perf$experiment = factor(perf$experiment)
  ...
  perf_plot <-
    ...
    labs(y = "Speedup", x = "") +
    scale_x_discrete(labels = printable_name) +
