...
MacGen$from_args(EffectCountArgumentsTotal = label_num(sum(total_effect_summary$argument_count)),
                 ...
                 EffectCountFunctionsTotal = label_num(length(unique(total_effect_summary$full_fun))),
                 EffectCountParametersTotal = label_num(length(unique(total_effect_summary$full_pos))))
```
...
```{r echo = FALSE, warning=FALSE, message=FALSE}
direct_effects %>%
filter(lookup_count == 0 & define_count != 0) %>%
