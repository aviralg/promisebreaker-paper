...
unevaluated <-
    ...
    summarize(eval_kind = case_when(call_count == 0 | force_tot == 0 ~ "Never",
...
MacGen$from_args(NeverCountPackages = length(unique(never_evaluated$pack_name)),
                 NeverCountFunctions = label_num(length(unique(never_evaluated$full_fun))),
                 NeverCountParameters = label_num(length(unique(never_evaluated$full_pos))))
                 
MacGen$from_args(SometimesCountPackages = length(unique(sometimes_evaluated$pack_name)),
                 SometimesCountFunctions = label_num(length(unique(sometimes_evaluated$full_fun))),
                 SometimesCountParameters = label_num(length(unique(sometimes_evaluated$full_pos))))

MacGen$from_args(UnevaluatedCountPackages = length(unique(unevaluated$pack_name)),
                 UnevaluatedCountFunctions = label_num(length(unique(unevaluated$full_fun))),
                 UnevaluatedCountParameters = label_num(length(unique(unevaluated$full_pos))))
                 
MacGen$from_args(UnevaluatedCountArguments = label_num(sum(sometimes_evaluated$arguments)))
