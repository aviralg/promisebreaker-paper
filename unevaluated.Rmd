...
unevaluated <-
    parameters %>%
    filter(!vararg) %>%
    mutate(call_count = call_count - missing) %>%
    group_by(pack_name, fun_name, formal_pos) %>%
    ## call count is 0 if call_count == missing.
    summarize(eval_kind = case_when(call_count == 0 ~ "Always Missing",
                                    force_tot == 0 ~ "Never",
                                    force_tot < call_count ~ "Sometimes",
                                    force_tot == call_count ~ "Always"),
              arguments = call_count - force_tot) %>%
    ungroup() %>%
    mutate(full_fun = paste0(pack_name, fun_name),
           full_pos = paste0(pack_name, fun_name, formal_pos)) %>%
    filter(eval_kind %in% c("Sometimes", "Never"))
    
datatable(unevaluated)

never_evaluated <- 
    unevaluated %>%
    filter(eval_kind == "Never")
    
sometimes_evaluated <- 
    unevaluated %>%
    filter(eval_kind == "Sometimes")
    
    
MacGen$from_args(UnevaluatedNeverCountPackages = length(unique(never_evaluated$pack_name)),
                 UnevaluatedNeverCountFunctions = label_num(length(unique(never_evaluated$full_fun))),
                 UnevaluatedNeverCountParameters = label_num(length(unique(never_evaluated$full_pos))))
                 
MacGen$from_args(UnevaluatedSometimesCountPackages = length(unique(sometimes_evaluated$pack_name)),
                 UnevaluatedSometimesCountFunctions = label_num(length(unique(sometimes_evaluated$full_fun))),
                 UnevaluatedSometimesCountParameters = label_num(length(unique(sometimes_evaluated$full_pos))))

MacGen$from_args(UnevaluatedTotalCountPackages = length(unique(unevaluated$pack_name)),
                 UnevaluatedTotalCountFunctions = label_num(length(unique(unevaluated$full_fun))),
                 UnevaluatedTotalCountParameters = label_num(length(unique(unevaluated$full_pos))))
                 
MacGen$from_args(UnevaluatedTotalCountArguments = label_num(sum(sometimes_evaluated$arguments)))
