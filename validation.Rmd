...
validation_summary <-
    ...
    arrange(order) %>%
    select(order, signature, total) %>%
    mutate(count = map_int(str_split(total, fixed("&")), ~as.integer(.[[1]])),
           perc = map_chr(str_split(total, fixed("&")), ~.[[2]]))
...
MacGen$from_args(ValidationAllProgramCount = label_num(nrow(all_programs)),
                 ValidationBaselineProgramCount = label_num(baseline_programs))

MacGen$from_args(ValidationStrictFailedProgramCount = label_num(validation_summary$count),
                 ValidationStrictFailedProgramPerc = validation_summary$perc)
